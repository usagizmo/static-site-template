...
export const setSmoothScroll = function () {
  ...
  const getScrollTop = function (hash) {
    ...
    if (!target) return 0
    const targetYOffset = target.getBoundingClientRect().top + window.pageYOffset
    const navOffset = document.querySelector(headerSelector).clientHeight + offset
    return targetYOffset - navOffset
  }

  gsap.utils.toArray('a[href^="#"]:not([data-modal-open])').forEach(function (el) {
